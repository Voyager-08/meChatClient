...
#include "networkmanager.h" // 新增：引入网络管理器头文件
#include <QTimer>
#include <QScrollBar>
...
void ChatWindow::setUserId(QString userID)
{
    this->userID = userID;
}
...
void ChatWindow::showContact()
{
    ...
    query.prepare(R"(
        SELECT f.friend_note, u.avatarpath, u.motto ,f.friend_id
        FROM friendships f 
    ...
    while (query.next())
    {
        ...
        QString friendID = query.value(3).toString();

        //添加联系人到本地列表
        contactList.append(Friend{friendID,friendNote, avatarPath, message});
...
void ChatWindow::sendMessage() // 发送消息
{
    ...
    messageDataMap[receiverID].append(msg);
}
...
void ChatWindow::onContactClicked(const QModelIndex &index) // 点击联系人
{
    ...
    QString contactName = index.data(static_cast<int>(ContactRoles::NameRole)).toString();
