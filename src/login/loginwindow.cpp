...
void LoginWindow::initUI()
{
    ...
    // 设用户头像
    userLabel = new QLabel(loginWidget);
    userLabel->setPixmap(QPixmap(registerWidget->avatarPath));
    userLabel->setGeometry(315, 30, 80, 80);// 设置位置和大小
    userLabel->setScaledContents(true);  // 自动缩放图片适应标签大小
    ...
    userLabel->setGraphicsEffect(shadowEf);// 设置阴影效果
...
{
    // 加载并缩放背景图以适应窗口大小
    QPixmap bg("D:/cxdownload/夸克文件下载/6.png");
    if (bg.isNull()) {
...
void LoginWindow::registerSuccess()
{
    ...
    userLabel->setPixmap(QPixmap(registerWidget->avatarPath));// 显示注册成功后的用户头像
}
...
void LoginWindow::onForgetPassword()
{
...
{
    qDebug()<<"-->userID:"<<userIDLineEdit->text()<<"\n-->password:"<<passwordLineEdit->text();// 输出调试信息
