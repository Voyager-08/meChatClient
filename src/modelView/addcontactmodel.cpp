// addcontactmodel.cpp
#include "addcontactmodel.h"
#include <QPixmap>
#include <QFile>
#include <QDebug> 

AddContactModel::AddContactModel(QObject *parent)// 显示构造函数，必须传父对象指针
    : QAbstractListModel(parent)// 调用父类构造函数
{
    
}

int AddContactModel::rowCount(const QModelIndex &parent) const// 获取行数
{
    Q_UNUSED(parent);  // 明确标记未使用的参数，避免编译器警告
    return m_contacts.count();// 返回联系人数量
}

QVariant AddContactModel::data(const QModelIndex &index, int role) const
{
...
QHash<int, QByteArray> AddContactModel::roleNames() const
{
...
void AddContactModel::addContact(const Model_Contact &contact)
{ //  添加联系人到模型中的实现函数
...
void AddContactModel::addContact(const QString &name, const QString &avatarPath,const QString &ID)
{
    beginInsertRows(QModelIndex(), rowCount(), rowCount()); //  通知视图即将插入新行，插入位置为当前最后一行之后
...
void AddContactModel::clearContacts()
{
