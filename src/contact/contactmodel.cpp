...
QVariant ContactModel::data(const QModelIndex &index, int role) const
{
    ...
    const Contact &contact = m_contacts.at(index.row()); //  获取指定索引位置的联系人引用
    ...
    case static_cast<int>(ContactRoles::MessageRole): //  当角色为消息角色时，返回联系人的消息内容
...
QHash<int, QByteArray> ContactModel::roleNames() const
{
    ...
    roles[static_cast<int>(ContactRoles::MessageRole)] = "message"; //  添加消息角色映射
...
void ContactModel::addContact(const QString &name, const QString &avatarPath, const QString &message,const QString &id)
{
    Contact contact{ name, avatarPath, message,id }; //  创建一个Contact对象，并使用传入的参数进行初始化
    addContact(contact); //  调用addContact函数，将Contact对象添加到模型中
}